<template>
  <ContainerGroup>
    <Container>
      <el-form :model="formInline" :inline="true" ref="formInline" label-width="90px">
          ...
          <el-form-item label="派工时间：" v-show="showAll">
            <el-date-picker value-format="yyyy-MM-dd" v-model="formInline.submitDate" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期">
            </el-date-picker>
          </el-form-item>
          ...
          <el-form-item label="运送类型：" v-show="showAll">
            <el-select placeholder="请选择" v-model="formInline.examType" clearable style="width:100%;">
              <el-option :value="0" label="送回"></el-option>
              <el-option :value="1" label="领取"> </el-option>
            </el-select>
            <!-- <el-select placeholder="请选择" clearable v-model="formInline.examType" style="width:100%;">
                <el-option v-for="(item,index) in examType" :key="index" :value="item.id" :label="item.name"></el-option>
              </el-select> -->
          </el-form-item>
          <el-form-item label="完成时间：" v-show="showAll">
            <el-date-picker value-format="yyyy-MM-dd" v-model="formInline.finishDate" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期">
            </el-date-picker>
          </el-form-item>
          ...
          <el-form-item label="物品：" v-show="showAll">
            <el-select placeholder="请选择" clearable v-model="formInline.goods" style="width:100%;">
              <el-option v-for="(item,index) in goods" :key="index" :value="item.dictId" :label="item.name"></el-option>
            </el-select>
          </el-form-item>
    ...
    <Container>
      ...
      <el-table isFillHeight stripe size="mini" :header-cell-style="{ background: '#FAFAFA', height: '42px' }" :data="tableData" border :height="tableHeight" v-loading="loading" @selection-change="handleSelectionChange">
        <el-table-column type="selection" width="55">
        </el-table-column>
        <el-table-column label="任务编号" width="140" prop="taskOrderNumber"></el-table-column>
        ...
        <el-table-column label="运送类型" show-overflow-tooltip prop="transportType">
          <template slot-scope="scope">
            <span v-if="scope.row.transportType !== null">
              {{scope.row.transportType === 1 ? '领取':'送回'}}
            </span>
          </template>
        </el-table-column>
        <el-table-column label="物品" prop="goodsNameList" min-width="150">
          <template slot-scope="scope">
        ...
        <el-table-column label="目的地" show-overflow-tooltip prop="toSpace"></el-table-column>
        <el-table-column label="运送/领取时间" show-overflow-tooltip prop="goodsTransportTime" width="110"></el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      init() {
        ...
        this.goods = this.initInfo.goods
      },
      // 展开收起搜索项
      closeSearch() {
      ...
      _getData() {
        ...
        let parms = {
          ...
          toSpace: this.formInline.destination,
          bedNo: this.formInline.sickbed,
          startGoodsTransportTime: this.formInline.transportDate[0],
          endGoodsTransportTime: this.formInline.transportDate[1],
          transportType: this.formInline.examType,
          goodsId: this.formInline.goods,
        }
        console.log(parms)
