...
import { pageListInit, getListPage, cancelDispatch, exportTaskOrderDispatchExcel } from './../../../apis/projectRegister/indexApi'
...
export default {
  ...
  methods: {
    ...
    selectionHandle () {
      ...
      if (this.excelBtns === '1') {
        exportTaskOrderDispatchExcel(params).then((res) => {
          if (res.status === 200) {
            ...
            downloadLink.download = "任务派工列表.xlsx"
            downloadLink.click()
        ...
        if (this.multVal === undefined || this.multVal.length === 0) {
          ...
          exportTaskOrderDispatchExcel({idList: idList}).then((res) => {
            if (res.status === 200) {
              let blob = new Blob([res.data], {
                type: res.data.type + "charset=utf-8",
              })
