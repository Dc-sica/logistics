<template>
  <button
    class="ck-button animate"
    size="mini"
    :class="'btn-' + (type||'primary') + ' ' + (loading ? 'un-clickable' : '')"
    @click="$emit('click')"
  >
    ...
    <template v-else>
      <i v-if="icons[icon]" :class="icons[icon]" style="padding-right: 5px" />
      <slot />
    </template>
  </button>
</template>
...
.btn-delete:hover{
  box-shadow: 0px 0px 6px  @dangerColor;
}
.un-clickable {
  opacity: 0.7;
  pointer-events: none;
