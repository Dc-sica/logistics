...
<script>
import { selectTaskOrderByTaskTypes, taskOrderHandler } from './../../../apis/projectRegister/indexApi'
...
export default {
  ...
  methods: {
    _goback () {
      this.$router.push('/projectAccept')
    },
    getDetail () {
    ...
    _reject () {
      let taskType = this.type === '陪检' ? '1':(this.type === '标本'? '2' : '3')
      let parms = {
      ...
      taskOrderHandler(parms).then(res => {
        if (res.data.code === 200) {
