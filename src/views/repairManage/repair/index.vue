  ...
  <ContainerGroup>
    ...
    <Container>
      ...
      <el-table :data="tableData" border :header-cell-style="{ background: '#FAFAFA', height: '42px' }" stripe size="mini" highlight-current-row isFillHeight height="0" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            ...
            <el-tooltip class="item" effect="dark" v-if="scope.row.repairStatus === 1" content="编辑" placement="top">
      ...
      <Drawer title="评价" :visible.sync="drawerView" @onOk="_onSave" @cancel="_drawerHandleClose" :footerBtns="[
        ...
        <div class="group">
          <span class="sp"><i style="color:red;">*</i> 评分：</span>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      _addItemHandle() {
        this.$router.push('/repairRegister/add')
      },
      _toDetailHandle(row) {
        this.$router.push({
        ...
        if (this.excelBtns === '1') {
          exportList(params).then((res) => {
            if (res.status === 200) {
              ...
              downloadLink.download = "报修列表.xlsx"
              downloadLink.click()
