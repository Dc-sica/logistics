...
import { pageListInit, getListPage, exportGoods } from './../../../apis/projectRegister/indexApi'
  ...
  methods: {
    ...
    _getData () {
      let parms = {
        ...
        startGoodsTransportTime: this.searchForm.transportTime?this.searchForm.transportTime[0]:'',
        endGoodsTransportTime: this.searchForm.transportTime?this.searchForm.transportTime[1]:'',
    ...
    _addItemHandle () {
      this.$router.push('/projectRegister/addGoods')
    },
    _toDetailHandle (row) {
      this.$router.push({
      ...
      if (this.excelBtns === '1') {
        exportGoods(params).then((res) => {
          if (res.status === 200) {
            ...
            downloadLink.download = "任务完工运送物品列表.xlsx"
            downloadLink.click()
        ...
        if (this.multVal === undefined || this.multVal.length === 0) {
          ...
          exportGoods({idList: idList}).then((res) => {
            if (res.status === 200) {
              let blob = new Blob([res.data], {
                type: res.data.type + "charset=utf-8",
              })
