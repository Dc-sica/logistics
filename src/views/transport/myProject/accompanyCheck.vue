      ...
      <el-form :inline="true"
        ...
        <el-form-item label="目的地：" v-if="showAll">
          <el-select placeholder="请选择"
            ...
            <el-option v-for="item in destinationList"
              :key="item.dictId"
              :label="item.name"
              :value="item.dictId"></el-option>
          </el-select>
...
import { pageListInit, getListPage, exportTaskKeep, completeOrder } from './../../../apis/projectRegister/indexApi'
      ...
      if (this.excelBtns === '1') {
        exportTaskKeep(params).then((res) => {
          if (res.status === 200) {
            ...
            downloadLink.download = "我的任务陪检列表.xlsx"
            downloadLink.click()
