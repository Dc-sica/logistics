...
import {
  ...
  getListPage,
  cancelDispatch,
  exportTaskOrderDispatchKeepExcel,
...
export default {
  ...
  methods: {
    ...
    selectionHandle() {
      ...
      if (this.excelBtns === "1") {
        exportTaskOrderDispatchKeepExcel(params).then((res) => {
          if (res.status === 200) {
            ...
            downloadLink.download = "任务派工陪检列表.xlsx";
            downloadLink.click();
        ...
        if (this.multVal === undefined || this.multVal.length === 0) {
          ...
          exportTaskOrderDispatchKeepExcel({ idList: idList }).then((res) => {
            if (res.status === 200) {
              let blob = new Blob([res.data], {
                type: res.data.type + "charset=utf-8",
              });
