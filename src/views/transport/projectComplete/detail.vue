  ...
  <ContainerGroup class="wrapper">
    ...
    <Container class="wrapper-table" height="fill" scrollY="true">
      <div class="form-box">
        ...
        <div class="form-row p-clear">
          <el-col :span="24">
            <div class="h40 fl"><span>照片凭证：</span></div>
            <div class="fileName fl">
              <div class="imgDiv" v-for="(item, index) in formInfo.taskOrderDetailImagesList" :key="index">
                <img :src="item" alt="">
              </div>
            </div>
          </el-col>
          <el-col :span="24">
            <div class="h40"><span>完成描述：</span>{{formInfo.taskOrderDetailRemark}}</div>
          </el-col>
          <el-col :span="24">
            <div class="h40"><span>签名：</span></div>
          </el-col>
        </div>
      </div>
    </Container>
...
export default {
  ...
  methods: {
    _goback () {
      this.$router.push('/projectComplete')
    },
    getDetail () {
