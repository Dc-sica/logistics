...
import { pageListInit, getListPage, exportTaskOrder, taskOrderCancel } from '../../../../apis/projectRegister/indexApi'
  ...
  methods: {
    ...
    _getData () {
      let parms = {
        ...
        startCreateTime: this.searchForm.sureTime
          ? this.searchForm.sureTime[0]
          : "",
        endCreateTime: this.searchForm.sureTime
    ...
    selectionHandle() {
      ...
      if (this.excelBtns === "1") {
        exportTaskOrder(params).then((res) => {
          if (res.status === 200) {
            ...
            downloadLink.download = "任务登记列表.xlsx";
            downloadLink.click();
        ...
        if (this.multVal === undefined || this.multVal.length === 0) {
          ...
          exportTaskOrder({ idList: idList }).then((res) => {
            if (res.status === 200) {
              let blob = new Blob([res.data], {
                type: res.data.type + "charset=utf-8",
              });
