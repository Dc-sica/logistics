          ...
          <template slot-scope="scope">
            ...
            >
              <el-button
                ...
                v-html="'&#xe65f;'"
                @click="_cancelApplyHandle(scope.row)"
              >
              </el-button
              >>
            </el-tooltip>
  ...
  methods: {
    ...
    init() {
      pageListInit().then((res) => {
        if (res.data.code === 200) {
          ...
          this.deptList = res.data.data.destination;
    ...
    _getData() {
      let parms = {
        ...
        startTransportTime: this.searchForm.sureTime
          ? this.searchForm.sureTime[0]
          : "",
        endTransportTime: this.searchForm.sureTime
    ...
    selectionHandle() {
      ...
      if (this.excelBtns === "1") {
        exportTaskOrderGoodExcel(params).then((res) => {
          if (res.status === 200) {
            ...
            downloadLink.download = "运送物品列表.xlsx";
            downloadLink.click();
        ...
        if (this.multVal === undefined || this.multVal.length === 0) {
          ...
          exportTaskOrderGoodExcel({ idList: idList }).then((res) => {
            if (res.status === 200) {
              let blob = new Blob([res.data], {
                type: res.data.type + "charset=utf-8",
              });
