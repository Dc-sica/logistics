...
<script>
import { selectTaskOrderByTaskTypes, listNursingGroupUser, dispatchOrder } from './../../../apis/projectRegister/indexApi'
...
export default {
  ...
  mounted () {
    ...
    if (userInfo) {
      const { name, nursingLeaderFlag } = JSON.parse(userInfo)
      this.name = name
  ...
  methods: {
    ...
    getNurseList () {
      listNursingGroupUser({
        ...
        obligationId: this.nursingLeaderFlag
      }).then(res => {
    ...
    getDetail () {
      selectTaskOrderByTaskTypes({
        ...
        if (res.data.code === 200) {
          ...
          this.nursingUserId = this.formInfo.userId
