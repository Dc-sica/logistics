  ...
  <ContainerGroup>
    <Container>
      <el-form :inline="true" ref="searchForm" :model="searchForm" label-width="auto">
        ...
        <el-form-item label="运送类型：" v-if="showAll">
          <el-select placeholder="请选择" v-model="searchForm.transportType">
            <el-option
        ...
        <el-form-item label="派工状态：" v-if="showAll">
          <el-select placeholder="请选择" v-model="searchForm.dispatchStatus">
            <el-option
    ...
    <Container>
      ...
      >
        ...
        <el-table-column label="目的地" prop="toSpace"></el-table-column>
        <el-table-column
          label="紧急程度"
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            ...
            <el-tooltip
              ...
              effect="dark"
              v-if="
                scope.row.dispatchStatus === 2 &&
                (scope.row.orderType === 1 || scope.row.orderType === 3)
              "
              content="取消派工"
              placement="top"
            >
...
import {
  ...
  getListPage,
  cancelDispatch,
  exportTaskOrderDispatchGoodExcel,
  ...
  methods: {
    ...
    _getData() {
      let parms = {
        ...
        startGoodsTransportTime: this.searchForm.goodsTransportTime
          ? this.searchForm.goodsTransportTime[0]
          : "",
        endGoodsTransportTime: this.searchForm.goodsTransportTime
          ? this.searchForm.goodsTransportTime[1]
          : "",
    ...
    selectionHandle() {
      ...
      if (this.excelBtns === "1") {
        exportTaskOrderDispatchGoodExcel(params).then((res) => {
          if (res.status === 200) {
            ...
            downloadLink.download = "任务派工运送物品列表.xlsx";
            downloadLink.click();
        ...
        if (this.multVal === undefined || this.multVal.length === 0) {
          ...
          exportTaskOrderDispatchGoodExcel({ idList: idList }).then((res) => {
            if (res.status === 200) {
              let blob = new Blob([res.data], {
                type: res.data.type + "charset=utf-8",
              });
