  ...
  <ContainerGroup>
    ...
    <Container>
      ...
      <el-table :data="tableData"
        ...
        <el-table-column label="受理状态" prop="orderStatusName">
          <template slot-scope="scope">
...
import { pageListInit, getListPage, exportTaskOrderHandleSampleExcel } from './../../../apis/projectRegister/indexApi'
...
export default {
  ...
  methods: {
    ...
    selectionHandle () {
      ...
      if (this.excelBtns === '1') {
        exportTaskOrderHandleSampleExcel(params).then((res) => {
          if (res.status === 200) {
            ...
            downloadLink.download = "任务受理运送标本列表.xlsx"
            downloadLink.click()
        ...
        if (this.multVal === undefined || this.multVal.length === 0) {
          ...
          exportTaskOrderHandleSampleExcel({idList: idList}).then((res) => {
            if (res.status === 200) {
              let blob = new Blob([res.data], {
                type: res.data.type + "charset=utf-8",
              })
