  ...
  <ContainerGroup class="wrapper">
    ...
    >
        ...
        <el-table-column label="任务类型"
          prop="taskTypeName"></el-table-column>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            ...
            <el-tooltip class="item"
              effect="dark"
              content="转单"
              v-if="scope.row.orderStatus === 5 && (scope.row.orderType === 2 || scope.row.orderType === 3) &&  nursingLeaderFlag === 0"
              placement="top">
              <el-button type="text"
                class="iconfont"
                v-html="'&#xe7d4;'"
...
import { pageListInit, getListPage, exportTask, completeOrder } from './../../../apis/projectRegister/indexApi'
  ...
  methods: {
    ...
    _getData () {
      let parms = {
        ...
        startCreateTime: this.searchForm.createTime?this.searchForm.createTime[0]:'',
        endCreateTime: this.searchForm.createTime?this.searchForm.createTime[1]:'',
      ...
      if (this.excelBtns === '1') {
        exportTask(params).then((res) => {
          if (res.status === 200) {
            ...
            downloadLink.download = "我的任务列表.xlsx"
            downloadLink.click()
        ...
        if (this.multVal === undefined || this.multVal.length === 0) {
          ...
          exportTask({idList: idList}).then((res) => {
            if (res.status === 200) {
              let blob = new Blob([res.data], {
                type: res.data.type + "charset=utf-8",
              })
