    ...
    <Container>
      <el-form ref="searchForm" :inline="true" :model="searchForm" label-width="auto">
        ...
        <el-form-item label="评价状态：" v-if="showAll">
          <el-select placeholder="请选择" v-model="searchForm.evaluation" clearable>
        ...
        <el-form-item label="接单类型：" v-if="showAll">
          <el-select placeholder="请选择" v-model="searchForm.orderType" clearable>
            <el-option v-for="item in orderType" :key="item.code" :label="item.value" :value="item.code"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="报修时间：" v-if="showAll">
          <el-date-picker v-model="searchForm.inTime" type="daterange" size="small" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" style="width:100%">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="派工时间：" v-if="showAll">
          <el-date-picker v-model="searchForm.sureTime" type="daterange" size="small" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" style="width:100%">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="完成时间：" v-if="showAll">
          <el-date-picker v-model="searchForm.finishTime" type="daterange" size="small" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" style="width:100%">
          </el-date-picker>
        </el-form-item>
    ...
    methods: {
      ...
      init() {
        pageListInit().then(res => {
          if (res.data.code === 200) {
            ...
            this.orderType = res.data.data.orderModel
      ...
      _getData() {
        let parms = {
          ...
          commentStatus: this.searchForm.evaluation,
          current: this.currentPage,
          pageSize: this.pageSize,
        }
        getSynthesisPage(parms).then(res => {
          if (res.data.code === 200) {
      ...
      selectionHandle() {
        ...
        if (this.excelBtns === '1') {
          exportSynthesis(params).then((res) => {
            if (res.status === 200) {
