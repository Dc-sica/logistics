    ...
    <Container height="fill" scrollY="true">
      ...
      <div class="form-box table-box">
        ...
        <div class="table-content">
          ...
          <el-table stripe size="mini" height="25vh" :header-cell-style="{ background: '#FAFAFA', height: '42px' }" :data="tableData" border @selection-change="handleSelectionChangeArr" highlight-current-row>
            ...
            <el-row :gutter="10">
              ...
              <el-col :span="8">
                <el-form-item label="品牌：">
                  <el-select style="width:100%;" placeholder="请选择" :disabled="true" multiple collapse-tagsclearable clearable v-model="formInfo.brandIdList">
            ...
            <el-row :gutter="40">
              ...
              <el-col :span="12">
                <el-form-item label="设备档案：">
                  ...
                  <div class="tip">支持扩展名：jpg, png, pdf, doc,docx</div>
                  <div class="fileName">
      ...
      <el-dialog v-if="visible" :close-on-click-modal="false" :visible="visible" class="dialog-wrapper" width="800px" title='选择参数' :before-close="_itemHandleClose">
        ...
        <div slot="footer">
          <el-button size="small" @click="_itemHandleClose">取消</el-button>
  ...
  export default {
    ...
    methods: {
        ...
        lookRecordById(param).then(res => {
          if (res.data.code == 200) {
            ...
            setTimeout(() => {
              ...
              if (result.archives && result.archives.length > 0) { //设备档案文件
              ...
              let arr = result.supplier && result.supplier.reduce((prev, current) => {
                prev.push(Number(current))
                return prev
              }, [])
              this.formInfo.supplier = arr

              let arrLable = result.labelIdList && result.labelIdList.reduce((prev, current) => {
                prev.push(Number(current))
                return prev
              }, [])

              this.formInfo.labelsList = arrLable
      ...
      handleGetPhonePerson(value) { //选择厂商填充厂家联系人、联系方式
        this.manufacturerList.forEach(item => {
          if (value === item.manufacturerId) {
            ...
            this.formInfo.brandIdList = item.brand && item.brand.split(',').map(item => {
              return Number(item)
            }) //品牌
      ...
      handleSubmitParam() { //参数弹窗提交
        ...
        // this.multipleParamSelection.forEach(item => {
        //   console.log('item',item)
        //   this.tableData.forEach(element => {
        //      console.log('element',element)
        //     if (element.paramId) {
        //       if (item.id == element.paramId) {
        //         console.log(1111111)
        //         this.$message.warning('不能提交重复数据!')
        //         return false
        //       }
        //     } else {
      ...
      handleDeletePreteme(row) { //参数列表删除
        if (row.relationId) {
          ...
          deleteParamRelation(param).then(res => {
            if (res.data.code == 200) {
      ...
      handleApproval(formName) { //确定
        ...
        if (this.tableData.length > 0) {
          this.tableData.forEach(item => {
            // console.log('item', item)
            paramListnew.push({
              paramId: item.id ? item.id : item.paramId,
              value: item.value
            })
          })
        }
        // console.log('paramListnew', paramListnew)
